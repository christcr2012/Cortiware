 ...
 * - Cookie: rs_user=<email> (legacy mv_user accepted during transition)
 * - Header: x-rs-user: <email> (legacy x-mv-user accepted)
 */
export function getEmailFromReq(req: NextApiRequest): string | null {
  const fromCookie = (req.cookies?.rs_user || req.cookies?.mv_user);
  const fromHeader = (req.headers["x-rs-user"] || req.headers["x-rsuser"] || req.headers["x-mv-user"] || req.headers["x-mvuser"]) as string | undefined;
