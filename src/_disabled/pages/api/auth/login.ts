...
function buildCookie(email: string) {
  let cookie = `rs_user=${encodeURIComponent(email)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=2592000`;
  ...
  try {
    ...
    try {
      ...
      if (providerAuthResult.success) {
        ...
        console.log(`🔧 Setting provider cookie: rs_provider=${emailInput}`);

        // Set PROVIDER-SPECIFIC cookie (different from client cookie)
        let providerCookie = `rs_provider=${encodeURIComponent(emailInput)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=2592000`;
    ...
    if (emailInput.toLowerCase() === HARDCODED_DEVELOPER_EMAIL.toLowerCase() && password === HARDCODED_DEVELOPER_PASSWORD) {
      ...
      let developerCookie = `rs_developer=${encodeURIComponent(emailInput)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=2592000`;
    ...
    if (emailInput.toLowerCase() === HARDCODED_ACCOUNTANT_EMAIL.toLowerCase() && password === HARDCODED_ACCOUNTANT_PASSWORD) {
      ...
      let accountantCookie = `rs_accountant=${encodeURIComponent(emailInput)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=2592000`;
